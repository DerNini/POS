...
public class CashRegisterJava {
    ...
    static class Core {
        ...
        private List<String[]> querySqlite(Path db, String sql) {
            List<String[]> out = querySqliteWithCli(db, sql);
            if (!out.isEmpty()) return out;
            return querySqliteWithPython(db, sql);
        }

        private void execSqlite(Path db, String sql) {
            if (execSqliteWithCli(db, sql)) return;
            execSqliteWithPython(db, sql);
        }

        private List<String[]> querySqliteWithCli(Path db, String sql) {
            try {
        ...
        private boolean execSqliteWithCli(Path db, String sql) {
            try {
        ...
        private List<String[]> querySqliteWithPython(Path db, String sql) {
            try {
                String script =
                        "import sqlite3,sys\n"
                                ...
                                + "con.close()\n";
                Process p = new ProcessBuilder("python", "-c", script, db.toString(), sql)
                        .redirectErrorStream(true)
                        .start();
                List<String> lines;
                try (BufferedReader br = new BufferedReader(new InputStreamReader(p.getInputStream()))) {
        ...
        private void execSqliteWithPython(Path db, String sql) {
            try {
                String script =
                        "import sqlite3,sys\n"
                                + "db,sql=sys.argv[1],sys.argv[2]\n"
                                ...
                                + "con.commit()\n"
                                + "con.close()\n";
                new ProcessBuilder("python", "-c", script, db.toString(), sql)
                        .redirectErrorStream(true)
                        .start()
                        .waitFor();
            } catch (Exception ignored) {}
